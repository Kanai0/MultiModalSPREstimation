...
- Loads the trained Keras model (.h5 or SavedModel)
- Rebuilds validation data using the same preprocessing as training
- Runs evaluation and prints/saves metrics and predictions

Usage examples:

  python ANN_eval_validation.py --model "C:/Users/Kanai/Desktop/MRtoSPR/Chang_Results/ANN_20250911_1156/model_final.h5"

  python ANN_eval_validation.py \
      --model ".../model_epoch_0020.h5" \
      --data-dir "C:/Users/Kanai/Synology/TWMU/0_張先生_共同研究_MRI阻止能/FromIzo/data_summarized" \
      --val-slc 40:50 --batch-size 16384
"""
...
from ANNmodel import (
    build_full_model,  # not directly used but kept for compatibility
    HybridLoss,
    compute_zn_volume,
    physics_ct_loss,
...
def parse_slice(s: str) -> slice:
    """Parse "start:end" to a Python slice."""
    if ":" not in s:
        raise argparse.ArgumentTypeError("Slice must be in 'start:end' format")
    a, b = s.split(":", 1)
