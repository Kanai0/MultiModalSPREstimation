...
def evaluate_model(model_path: str,
                   data_dir: str,
                   val_slc: slice = slice(40, 50),
                   batch_size: int = 16384,
                   out_dir: Optional[str] = None,
                   try_compiled: bool = True) -> Tuple[Dict, str, Dict]:
    """
    Core evaluation routine.

    Returns
    -------
    results: dict (metrics and counts)
    out_dir: str (where artifacts were saved)
    arrays: dict with keys 'rho_pred', 'rho_true', 'ct_pred', 'ct_true'
    """
    ...
    arrays = {
        'rho_pred': pred_vol,
        'rho_true': true_vol,
        'ct_pred': ct_pred.reshape(d_val, h, w),
        'ct_true': ct_true.reshape(d_val, h, w),
    }

    print("[Done] Evaluation complete.")
    print("- Output dir:", out_dir)
    print("- Metrics saved:", metrics_path)
    print("- Rho RMSE:", rmse_rho, "/ MAE:", mae_rho, "/ R2:", r2_rho)
    print("- CT  RMSE:", rmse_ct, "/ MAE:", mae_ct)

    return results, out_dir, arrays


# %% Parameters (edit here for Interactive or script runs)
MODEL_PATH: Optional[str] = None  # e.g., r"C:/Users/Kanai/Desktop/MRtoSPR/Chang_Results/ANN_YYYYMMDD_HHMM/model_final.h5"
DATA_DIR: str = r"C:/Users/Kanai/Synology/TWMU/0_張先生_共同研究_MRI阻止能/FromIzo/data_summarized/"
VAL_SLC: slice = slice(40, 50)
BATCH_SIZE: int = 16384
OUT_DIR: Optional[str] = None  # None -> auto under model folder
TRY_COMPILED: bool = True

# Placeholders for results (populated after running)
EVAL_RESULTS = None
EVAL_OUT_DIR = None
RHO_PRED_VAL = None
RHO_TRUE_VAL = None
...
# %% Interactive helper
def _run_interactive_eval():
    global EVAL_RESULTS, EVAL_OUT_DIR, RHO_PRED_VAL, RHO_TRUE_VAL, CT_PRED_VAL, CT_TRUE_VAL
    if not MODEL_PATH:
        print("[Hint] Set MODEL_PATH to your trained model file before running.")
        return
    results, out_dir, arrays = evaluate_model(
        model_path=MODEL_PATH,
        data_dir=DATA_DIR,
        val_slc=VAL_SLC,
        batch_size=BATCH_SIZE,
        out_dir=OUT_DIR,
        try_compiled=TRY_COMPILED,
    )
    EVAL_RESULTS = results
    EVAL_OUT_DIR = out_dir
    RHO_PRED_VAL = arrays['rho_pred']
    RHO_TRUE_VAL = arrays['rho_true']
    CT_PRED_VAL = arrays['ct_pred']
    CT_TRUE_VAL = arrays['ct_true']
    print("[Summary] Rho RMSE=", results['rho_metrics']['rmse'], "CT RMSE=", results['ct_metrics']['rmse'])


# %% Script entry (no argparse)
_IN_IPYTHON = False
...
if __name__ == "__main__" and not _IN_IPYTHON:
    if not MODEL_PATH:
        print("[Hint] Set MODEL_PATH at the top of the script to your trained model file.")
    else:
        _results, _out_dir, _arrays = evaluate_model(
            model_path=MODEL_PATH,
            data_dir=DATA_DIR,
            val_slc=VAL_SLC,
            batch_size=BATCH_SIZE,
...
# %% Save slice PNGs
# Configure which slice to save and intensity range.
SAVE_SLICE_INDEX: int = 0        # which D-index in validation volume to save
SAVE_VMIN: Optional[float] = None # None -> auto from min of both slices
SAVE_VMAX: float = 1.5            # specify display max
SAVE_DPI: int = 150


def save_val_slice_pngs():
    """Save specified slice from rho_pred_val and rho_true_val as PNGs.

    Uses global arrays populated after evaluation. Files are saved into
    `EVAL_OUT_DIR` as:
      - rho_pred_val_slice_{idx:03d}.png
      - rho_true_val_slice_{idx:03d}.png
    """
    global RHO_PRED_VAL, RHO_TRUE_VAL, EVAL_OUT_DIR
    if RHO_PRED_VAL is None or RHO_TRUE_VAL is None or EVAL_OUT_DIR is None:
        print("[Hint] Run evaluation first (_run_interactive_eval) to populate arrays.")
        return

    d, h, w = RHO_PRED_VAL.shape
    idx = int(SAVE_SLICE_INDEX) % d

    # Determine vmin/vmax
    if SAVE_VMIN is None:
        vmin = float(np.nanmin([np.nanmin(RHO_PRED_VAL[idx]), np.nanmin(RHO_TRUE_VAL[idx])]))
    else:
        vmin = float(SAVE_VMIN)
    vmax = float(SAVE_VMAX)

    out_pred = os.path.join(EVAL_OUT_DIR, f"rho_pred_val_slice_{idx:03d}.png")
    out_true = os.path.join(EVAL_OUT_DIR, f"rho_true_val_slice_{idx:03d}.png")
    ...
    for arr, path, title in [
        (RHO_PRED_VAL[idx], out_pred, "rho_pred"),
        (RHO_TRUE_VAL[idx], out_true, "rho_true"),
    ]:
